...
- **API docs** — springdoc OpenAPI / Swagger UI, bean validation and structured error responses via a
 ...
 - **Quality gates** — `./mvnw verify` enforces JaCoCo coverage (LINE ≥ 60%, BRANCH ≥ 60%), SpotBugs
   ...
   annotations; beans are wired explicitly in `infra/config`. **Living specifications:** every
  Business Component declares its EARS requirements in `package-info.java` — all six are
  spec-complete (RateLimiting, Cache, Auth, Analytics, Persistence, UrlShortener; 35 requirements,
  34 traced, 97%); tests link to requirements via `@TracesRequirement` and
  `scripts/check-living-spec.sh` (+ self-test) enforces ≥ 90% traceability in CI and at tag time.
