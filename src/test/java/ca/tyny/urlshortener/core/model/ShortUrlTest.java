...
class ShortUrlTest {
    ...
    private ShortUrl base() {
        return new ShortUrl(ID, URL, LocalDateTime.now(), "user1", true);
    }
    ...
    void expiresAtDefaultsToNull() {
        ...
        assertThat(new ShortUrl(ID, URL, LocalDateTime.now()).expiresAt()).isNull();
    }
    ...
    void withExpiresAtPreservesFields() {
        ...
        ShortUrl original = new ShortUrl(ID, URL, createdAt, "user1", true, 7, null);
