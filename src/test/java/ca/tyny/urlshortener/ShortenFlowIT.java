...
@DisplayName("URL Shortener E2E Tests")
class ShortenFlowIT extends BaseIntegrationTest {
  ...
  void shouldReturn404ForNonExistentUrl() {
    given().redirects().follow(false).when().get("/nonexistent123").then().statusCode(404);
  }
        ...
        .forEach(
            i -> {
              String id =
                  given()
                      .contentType(ContentType.JSON)
                      .body(new ShortenRequest("https://example.com/concurrent/" + i, null))
                      .post("/api/v1/urls")
                      .then()
                      .statusCode(200)
                      .extract()
  ...
  void shouldProtectAgainstInvalidIds() {
    ...
    for (String invalidId : invalidIds) {
      given().redirects().follow(false).get("/" + invalidId).then().statusCode(404);
    }
  }
}
