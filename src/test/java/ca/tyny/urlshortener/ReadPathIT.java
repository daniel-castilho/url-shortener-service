...
class ReadPathIT extends BaseIntegrationTest {
  ...
  @Autowired private UrlCachePort urlCache;
  ...
  void bloomNegativeTriggersFindByIdPerPolicyB() {
    ...
    given().redirects().follow(false).get("/" + nonExistentId).then().statusCode(404);
  ...
  void cacheLookupReturnsExplicitBloomNegativeForRejectedCodes() {
    ...
    assertThat(lookup.absence())
        .isEqualTo(ca.tyny.urlshortener.core.model.CacheLookup.Absence.BLOOM_NEGATIVE);
