...
class LinkResourceIT extends BaseIntegrationTest {
  ...
  void list_unauthenticatedReturns401() {
    given().get("/api/v1/urls").then().statusCode(401);
  }
  ...
  void get_unauthenticatedReturns401() {
    given().get("/api/v1/urls/" + linkId).then().statusCode(401);
  }
  ...
  void delete_archivedRedirectReturns404() {
    ...
    given().redirects().follow(false).get("/" + linkId).then().statusCode(404);
  }
  ...
  void delete_unauthenticatedReturns401() {
    given().delete("/api/v1/urls/" + linkId).then().statusCode(401);
  }
