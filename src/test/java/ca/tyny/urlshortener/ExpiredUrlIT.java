...
class ExpiredUrlIT extends BaseIntegrationTest {
  ...
  void shouldExpireAfterTtl() throws InterruptedException {
    ...
    given().redirects().follow(false).when().get("/" + id).then().statusCode(302);
  ...
  void shouldRedirectUrlWithoutExpiry() {
    ...
    mongoTemplate.save(
        new ShortUrlEntity(
            id, "https://www.example.com/forever", "hash1234", LocalDateTime.now(), null, false));
