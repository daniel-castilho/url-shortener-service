...
class DomainRedirectIT extends BaseIntegrationTest {
    ...
    void setUp() {
        ...
        customDomainRepository.save(new CustomDomain(BOUND_HOST, "user-1", DomainStatus.ACTIVE,
                "url-shortener-verify=deadbeef", Instant.now()));
        customDomainRepository.save(new CustomDomain(OTHER_HOST, "user-1", DomainStatus.ACTIVE,
                "url-shortener-verify=deadbeef", Instant.now()));
        ...
        urlRepository.save(new ShortUrl("bound01", "https://target.example.com/bound",
                LocalDateTime.now(), "user-1", false, 0, null, null, null, null, null, BOUND_HOST));
        urlRepository.save(new ShortUrl("plain01", "https://target.example.com/plain",
                LocalDateTime.now(), "user-1", false, 0, null, null, null, null, null, null));
    }
    ...
    void domainLessLinkRedirectsOnDefaultHostOnly() {
        ...
        given().redirects().follow(false)
                .header("Host", BOUND_HOST).get("/plain01")
                .then().statusCode(404);

        given().redirects().follow(false)
                .header("Host", OTHER_HOST).get("/plain01")
                .then().statusCode(404);
    }
    ...
    void boundLinkRedirectsUnderOwnHostOnly() {
        given().redirects().follow(false)
                .header("Host", BOUND_HOST).get("/bound01")
                .then().statusCode(302)
        ...
        given().redirects().follow(false)
                .header("Host", OTHER_HOST).get("/bound01")
                .then().statusCode(404);
    }
    ...
    void unknownHostResolvesNothing() {
        given().redirects().follow(false)
                .header("Host", "random.example.net").get("/plain01")
                .then().statusCode(404);
        given().redirects().follow(false)
                .header("Host", "random.example.net").get("/bound01")
                .then().statusCode(404);
    }
    ...
    void hostWithPortResolves() {
        ...
        given().redirects().follow(false)
                .header("Host", hostWithPort).get("/plain01")
                .then().statusCode(302)
                .header("Location", "https://target.example.com/plain");
    }
}