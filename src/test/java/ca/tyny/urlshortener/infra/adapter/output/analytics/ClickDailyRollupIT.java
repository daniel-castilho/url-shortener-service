...
class ClickDailyRollupIT extends BaseIntegrationTest {
  ...
  @Autowired private ClickDailyRollupJob rollupJob;
  ...
  void aggregatesYesterday() {
    ...
    mongoTemplate.insert(
        new ClickEventDocument(
            "aaa111", start, "UA-1", "203.0.113.1", "https://ref.example.com", "mobile", "BR"),
        MongoCollections.CLICK_EVENTS);
    mongoTemplate.insert(
        new ClickEventDocument(
            "aaa111", mid, "UA-2", "203.0.113.2", "https://ref.example.com", "mobile", null),
        MongoCollections.CLICK_EVENTS);
    mongoTemplate.insert(
        new ClickEventDocument(
            "aaa111", end.minusSeconds(60), "UA-3", "203.0.113.3", null, "desktop", "US"),
        MongoCollections.CLICK_EVENTS);
    // A different day must NOT be picked up
    mongoTemplate.insert(
        new ClickEventDocument(
            "aaa111", Instant.now(), "UA-x", "203.0.113.9", null, "mobile", "BR"),
        MongoCollections.CLICK_EVENTS);
  ...
  void rerunIsIdempotent() {
    ...
    Instant start = yesterday.atStartOfDay(ZoneOffset.UTC).toInstant();
    mongoTemplate.insert(
        new ClickEventDocument("bbb222", start, "UA-1", "203.0.113.1", null, "mobile", null),
        MongoCollections.CLICK_EVENTS);
    ...
    List<ClickDailyDocument> rows =
        mongoTemplate.find(
            Query.query(Criteria.where("shortCode").is("bbb222")), ClickDailyDocument.class);
