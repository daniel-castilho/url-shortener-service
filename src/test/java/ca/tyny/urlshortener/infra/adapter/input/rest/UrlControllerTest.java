...
class UrlControllerTest {
  ...
  void shouldShortenUrl() throws Exception {
    ...
    when(shortenUrlUseCase.shorten(eq(TEST_URL), isNull(), isNull(), (Long) isNull(), isNull()))
        .thenReturn(shortUrl);
  ...
  void shouldHandleCustomAlias() throws Exception {
    ...
    when(shortenUrlUseCase.shorten(
            eq(TEST_URL), eq(customAlias), isNull(), (Long) isNull(), isNull()))
        .thenReturn(shortUrl);
    ...
    verify(shortenUrlUseCase)
        .shorten(eq(TEST_URL), eq(customAlias), isNull(), (Long) isNull(), isNull());
  }
  ...
  void shouldReturn404WhenUrlNotFound() throws Exception {
    ...
    // When/Then
    mockMvc.perform(get("/" + TEST_ID)).andExpect(status().isNotFound());
  ...
  void shouldReturn429WithRetryAfterWhenRedirectLimited() throws Exception {
    when(rateLimiter.tryAcquire(
            eq(ca.tyny.urlshortener.core.ports.outgoing.RateLimitScope.REDIRECT), anyString()))
  ...
  void shouldReturn409WhenAliasAlreadyExists() throws Exception {
    ...
    when(shortenUrlUseCase.shorten(
            eq(TEST_URL), eq(customAlias), isNull(), (Long) isNull(), isNull()))
        .thenThrow(
            new ca.tyny.urlshortener.core.exception.AliasAlreadyExistsException(customAlias));
  ...
  void shouldPassResolvedExpiryForValidTtl() throws Exception {
    ...
    org.mockito.ArgumentCaptor<Long> ttlSeconds = org.mockito.ArgumentCaptor.forClass(Long.class);
    verify(shortenUrlUseCase)
        .shorten(eq(TEST_URL), isNull(), isNull(), ttlSeconds.capture(), isNull());
  ...
  void shouldRejectTtlOverCap() throws Exception {
    String body =
        "{\"originalUrl\":\"" + TEST_URL + "\",\"ttlSeconds\":" + (MAX_TTL_SECONDS + 1) + "}";
  ...
  @Test
  @DisplayName("POST /api/v1/urls should pass a custom domain through to the use case")
  ...
  @Test
  @DisplayName("POST /api/v1/urls should reject a non-positive ttlSeconds with 400")
  ...
  private static void assertThatWithinRoughly(
      java.time.Instant actual, java.time.Instant expected, long toleranceSecs) {
