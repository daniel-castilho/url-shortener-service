...
class DomainBindingIT extends BaseIntegrationTest {
    ...
    void setUp() {
        ...
        String user1Id = userRepository.findByEmail("user1@test.com")
                .map(ca.tyny.urlshortener.core.model.User::id)
                .orElseThrow();

        customDomainRepository.save(new CustomDomain(BOUND_HOST, user1Id, DomainStatus.ACTIVE,
    ...
    @Test
    @DisplayName("shorten with an owned verified domain: link bound and redirect resolves under the host only")
    ...
    void shortenWithForeignDomainIsForbidden() {
        given()
                ...
                .contentType(ContentType.JSON)
                .body("{\"originalUrl\":\"https://target.example.com/x\",\"domain\":\"" + BOUND_HOST + "\"}")
                .post("/api/v1/urls")
                .then().statusCode(403);
    }
    ...
    void shortenWithPendingDomainIsBadRequest() {
        String user1Id = userRepository.findByEmail("user1@test.com")
                .map(ca.tyny.urlshortener.core.model.User::id)
                .orElseThrow();
        customDomainRepository.save(new CustomDomain("pending.example.com", user1Id, DomainStatus.PENDING,
                "url-shortener-verify=deadbeef", Instant.now()));

        given()
                .header("Authorization", "Bearer " + user1Token)
                .contentType(ContentType.JSON)
                .body("{\"originalUrl\":\"https://target.example.com/x\",\"domain\":\"pending.example.com\"}")
                .post("/api/v1/urls")
                .then().statusCode(400)
    ...
    void shortenWithUnclaimedDomainIsBadRequest() {
        given()
                .header("Authorization", "Bearer " + user1Token)
                .contentType(ContentType.JSON)
                .body("{\"originalUrl\":\"https://target.example.com/x\",\"domain\":\"nobody.example.com\"}")
                .post("/api/v1/urls")
                .then().statusCode(400)
