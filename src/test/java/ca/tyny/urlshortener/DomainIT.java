...
class DomainIT extends BaseIntegrationTest {
    ...
    @MockitoBean
    private DnsTxtResolver dnsTxtResolver;
    ...
    void claim_returnsPendingWithToken() {
        DomainResponse response = given()
                .header("Authorization", "Bearer " + user1Token)
                .contentType(ContentType.JSON)
                .body("{\"host\":\"links.example.com\"}")
                .post("/api/v1/domains")
                .then().statusCode(201)
                .extract().as(DomainResponse.class);
    ...
    void list_returnsOwnDomains() {
        ...
        DomainListResponse user1 = given()
                .header("Authorization", "Bearer " + user1Token)
                .get("/api/v1/domains")
                .then().statusCode(200)
                .extract().as(DomainListResponse.class);

        assertThat(user1.domains()).extracting(DomainResponse::host)
    ...
    void delete_removesOwnedHost() {
        ...
        DomainListResponse empty = given()
                .header("Authorization", "Bearer " + user1Token)
                .get("/api/v1/domains")
                .then().statusCode(200)
                .extract().as(DomainListResponse.class);
