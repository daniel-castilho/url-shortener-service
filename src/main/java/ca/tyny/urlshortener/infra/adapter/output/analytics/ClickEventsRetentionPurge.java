 ...
 *
 * <p>Runs on a configurable schedule (default daily at 02:00 UTC) and deletes events older than
 * {@code app.analytics.retention-days} (default 90 days) in small batches to avoid long-running
 * transactions and blocking the write path. The purge is idempotent and safe to run concurrently.
 *
 * <p>Metrics: - {@code analytics.retention.purged.total} — total events deleted across runs -
 * {@code analytics.retention.runs.total} — number of purge executions - {@code
 * analytics.retention.errors.total} — purge runs that failed
 */
@Component
public class ClickEventsRetentionPurge {
  ...
  public ClickEventsRetentionPurge(
    ...
    this.purgedCounter =
        Counter.builder("analytics.retention.purged.total")
            .description("Click events purged by the retention policy")
            .tag("pipeline", "clicks")
            .register(meterRegistry);
    this.runsCounter =
        Counter.builder("analytics.retention.runs.total")
            .description("Retention purge executions")
            .tag("pipeline", "clicks")
            .register(meterRegistry);
    this.errorsCounter =
        Counter.builder("analytics.retention.errors.total")
   ...
   *
   * <p>The purge deletes documents where {@code timestamp} is older than the retention window, in
   * batches of {@value #BATCH_SIZE}, stopping after {@value #MAX_RUN_MS} milliseconds to avoid
  ...
  public void purge() {
    ...
    log.info(
        "Starting click_events retention purge: retentionDays={}, cutoff={}",
        retentionDays,
    ...
    try {
      ...
      log.info(
          "Retention purge completed: deleted {} events (total this run: {})",
          deleted,
          totalDeleted);
    } catch (Exception e) {
  ...
  long deleteBatch(long cutoffMillis, int limit) {
    Query query =
        Query.query(Criteria.where("timestamp").lt(Instant.ofEpochMilli(cutoffMillis)))
            .limit(limit);
