 ...
 *
 * <p>Replaces the in-memory LinkedBlockingQueue: events survive restarts and bursts, and the stream
 * is bounded via approximate MAXLEN trimming.
 *
 * <p>Policy (locked): fire-and-forget + fail-open. track() never blocks the redirect and never
 * throws — on Redis failure the event is dropped and counted (analytics is non-critical path).
 */
@Component
public class RedisClickEventQueue implements AnalyticsPort {
  ...
  public RedisClickEventQueue(
    ...
    this.enqueuedCounter =
        Counter.builder("analytics.events.enqueued.total")
            .description("Click events durably enqueued onto the Redis Stream")
            .tag("pipeline", "clicks")
            .register(meterRegistry);
    this.droppedCounter =
        Counter.builder("analytics.events.dropped.total")
            .description("Click events dropped (Redis unavailable or stream write failed)")
            .tag("pipeline", "clicks")
            .register(meterRegistry);
    Gauge.builder("analytics.queue.depth", this, RedisClickEventQueue::streamLength)
  ...
  /**
   * Current stream depth for the metrics gauge. Fail-open: reports 0 when Redis is unavailable so
   * scraping never breaks.
   */
  long streamLength() {
    try {
  ...
  /**
   * Maps a domain event to its stream payload. Timestamps are normalized to UTC instants at this
   * boundary; the domain keeps LocalDateTime.
   */
