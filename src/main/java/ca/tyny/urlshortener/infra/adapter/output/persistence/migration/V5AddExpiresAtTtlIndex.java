...
/**
 * Adds a TTL index on {@code expiresAt} for {@code short_urls}. MongoDB's TTL monitor lazily
 * deletes documents once {@code expiresAt} has passed — the application-level expiry check remains
 * the source of truth. Idempotent via {@code ensureIndex}.
 */
@Component
public class V5AddExpiresAtTtlIndex implements SchemaMigration {
  ...
  public void apply(MongoTemplate mongoTemplate) {
    ...
    indexOps.ensureIndex(new Index("expiresAt", Sort.Direction.ASC).expire(0, TimeUnit.SECONDS));
  }
}
