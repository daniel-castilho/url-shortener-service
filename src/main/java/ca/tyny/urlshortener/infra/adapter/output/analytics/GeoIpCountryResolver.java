 ...
 *
 * <p>Purely a privacy/analytics enrichment: {@link #countryForIp(String)} never blocks on hard
 * failure and always returns {@code null} when disabled, misconfigured, missing the database file,
 * seeing a private/internal IP, or on any lookup error. Slow/failed lookups must never slow the
 * batch.
 */
public class GeoIpCountryResolver {
  ...
  /**
   * Loads the MaxMind country database from disk. Fails open: any load error yields a disabled
   * resolver so analytics never breaks startup.
   */
  public static GeoIpCountryResolver fromFile(String maxmindDbPath) {
    ...
    try {
      return new GeoIpCountryResolver(
          true, new DatabaseReader.Builder(new File(maxmindDbPath)).build());
    } catch (Exception e) {
      log.warn(
          "Could not load MaxMind database from '{}' — geo enrichment disabled: {}",
          maxmindDbPath,
          e.getMessage());
      return disabled();
    }
