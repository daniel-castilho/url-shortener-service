...
/**
 * Performs the actual DNS verification for a custom domain and drives the {@code PENDING → ACTIVE /
 * FAILED} transition.
 *
 * <p>Best-effort by design: a lookup failure is a FAILED check, never an exception that aborts the
 * batch or the scheduled run.
 */
@Component
public class DomainVerificationService {
  ...
  /**
   * Verifies {@code host} now: resolves its TXT records, promotes to ACTIVE when the verification
   * token is present, otherwise FAILS the check and removes the host from the active registry.
   *
