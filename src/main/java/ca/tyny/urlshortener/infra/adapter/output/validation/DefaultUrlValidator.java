 ...
 *
 * <ul>
 *   <li>Enforces HTTPS by default (configurable via {@code app.url.allow-http})
 *   <li>Validates host syntax and structure
 *   <li>Resolves DNS and blocks private/internal/metadata IPs (configurable)
 *   <li>Caches DNS resolutions with TTL to avoid repeated lookups
 * </ul>
 */
@Component
public class DefaultUrlValidator implements UrlValidator {
  ...
  private static final Set<String> METADATA_IPS =
      Set.of(
          ...
          "fd00::1",
          "fe80::1", // IPv6 link-local common
          "100.100.100.200", // Alibaba Cloud metadata
          "169.254.169.250" // Azure IMDS
          );

  private static final List<CidrBlock> PRIVATE_CIDRS =
      List.of(
  ...
  public UrlValidator.ValidationResult doValidate(String rawUrl) {
    ...
    try {
      ...
      return UrlValidator.ValidationResult.blocked(
          null, List.of(), "Invalid URL format: " + e.getMessage());
    }
    ...
    if (scheme == null || (!scheme.equalsIgnoreCase("http") && !scheme.equalsIgnoreCase("https"))) {
      return UrlValidator.ValidationResult.blocked(
          null, List.of(), "URL must use http:// or https:// scheme");
    }
    if (!allowHttp && scheme.equalsIgnoreCase("http")) {
      return UrlValidator.ValidationResult.blocked(
          null, List.of(), "HTTP URLs are not allowed; use HTTPS");
    }
  ...
  private List<InetAddress> resolveWithCache(String host) {
    ...
    // Cache the result
    dnsCache.put(
        host, new DnsCacheEntry(ips, System.currentTimeMillis() / 1000 + 300)); // 5 min TTL
    return ips;
  }
