 ...
 *
 * <p>Host uniqueness is enforced by the unique {@code host} index created in migration V8; a
 * duplicate save surfaces as {@link DuplicateKeyException} and is mapped to the domain exception
 * (never a check-then-put race).
 */
@Repository
  ...
  public List<CustomDomain> findByUserId(String userId) {
    try {
      return mongoTemplate
          .find(Query.query(Criteria.where("userId").is(userId)), CustomDomainEntity.class)
          .stream()
          .map(mapper::toDomain)
          .toList();
    } catch (Exception e) {
