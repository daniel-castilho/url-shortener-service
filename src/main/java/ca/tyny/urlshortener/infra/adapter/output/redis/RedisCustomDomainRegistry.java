 ...
 * short-lived in-memory snapshot (default refresh interval ~5s) so {@code isActiveHost} is
 * a cheap, non-blocking local check. Redis is the source for the snapshot; when unavailable
 * the snapshot falls back to the persistence layer to avoid a hard failure on the hot path
 * (best-effort, self-healing via {@link #refresh()}).
 */
@Component
public class RedisCustomDomainRegistry implements CustomDomainRegistryPort {
    ...
    public RedisCustomDomainRegistry(StringRedisTemplate redis,
    ...
    public void refresh() {
        ...
        try {
            ...
            log.warn("Redis snapshot failed for active custom domains; falling back to Mongo: {}",
                    e.getMessage());
