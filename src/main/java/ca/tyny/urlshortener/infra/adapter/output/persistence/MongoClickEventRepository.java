 ...
 *
 * <p>Bulk-inserts analytics events; no reads are exposed here yet (aggregation queries are a future
 * concern). Failures surface as RepositoryException so the worker can apply its bounded retry
 * policy.
 */
@Repository
  ...
  public void insertAll(List<ClickEventDocument> events) {
    ...
    try {
      mongoTemplate.insert(
          events,
          ca.tyny.urlshortener.infra.adapter.output.persistence.config.MongoCollections
              .CLICK_EVENTS);
