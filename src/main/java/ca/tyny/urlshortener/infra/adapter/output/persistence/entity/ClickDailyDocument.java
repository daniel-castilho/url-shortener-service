 ...
 *
 * <p>One row per {@code (shortCode, day)} pair, computed by the scheduled aggregation job from the
 * raw {@code click_events} collection. {@code day} is the UTC calendar day ({@code yyyy-MM-dd}).
 * {@code breakdown} maps a dimension ({@code device | country | referrer}) to a value-to-count map.
 * The {@code (shortCode, day)} pair is unique (V9) so re-runs of the rollup overwrite the row
