...
/**
 * Legacy index cleanup: drops the unique index on {@code originalUrl} (no URL deduplication — the
 * same long URL may be shortened to distinct codes). The index does not exist on fresh databases,
 * so dropping is best-effort and must never fail.
 */
@Component
