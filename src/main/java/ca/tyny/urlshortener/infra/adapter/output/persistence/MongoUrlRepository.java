 ...
 *
 * <p>Responsibilities: - Persist and retrieve shortened URLs from MongoDB - Convert between domain
 * model (ShortUrl) and persistence entity (ShortUrlEntity) - Encapsulate MongoDB-specific
 * exceptions
 *
 * <p>Implements three ports: - {@link UrlRepositoryPort}: used by the shortener/redirect write path
 * - {@link LinkQueryPort}: query operations for the links-as-resource API - {@link
 * LinkMutationPort}: mutation operations for the links-as-resource API
 *
 * <p>Patterns applied: - Repository Pattern: implements UrlRepositoryPort, LinkQueryPort,
 * LinkMutationPort - Adapter Pattern: adapts MongoTemplate to the ports - Circuit Breaker:
 * resilience to database failures - Mapper Pattern: converts domain ↔ entity
 */
@Repository
public class MongoUrlRepository implements UrlRepositoryPort, LinkQueryPort, LinkMutationPort {
   ...
   *
   * <p>Uses a server-side $inc so concurrent increments are never lost. A missing code is a no-op
  ...
  public PageResult<ShortUrl> findByUserId(String userId, int limit, Cursor cursor) {
    try {
      ...
      List<ShortUrl> items = entities.stream().map(mapper::toDomain).toList();
