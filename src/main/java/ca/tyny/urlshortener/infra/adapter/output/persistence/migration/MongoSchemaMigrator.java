 ...
 *
 * <p>Applies {@link SchemaMigration}s once, in ascending version order, on application startup, and
 * records each application in the {@code schema_migrations} history collection (one document per
 * version). The runner is fail-fast: if a migration throws, startup aborts and the failed migration
 * is never recorded as applied.
 *
 * <p>Each migration must be idempotent so that a fresh database (or a wiped history) is migrated to
 * the current schema with a single pass. If the recorded checksum of an already-applied version no
 * longer matches the current class, an idempotent migration is re-applied and its record updated; a
 * non-idempotent one aborts startup.
 */
@Component
public class MongoSchemaMigrator {
  ...
  public MongoSchemaMigrator(
      MongoTemplate mongoTemplate, List<SchemaMigration> migrations, MetricsPort metrics) {
    this.mongoTemplate = mongoTemplate;
    this.migrations =
        migrations.stream().sorted(Comparator.comparingInt(SchemaMigration::version)).toList();
  ...
  public void migrate() {
    ...
    for (SchemaMigration migration : migrations) {
      ...
      if (existing != null) {
        log.warn(
            "Re-applying idempotent schema migration V{} ({}) after checksum change",
            version,
            migration.description());
      }
      ...
      try {
        ...
        log.error(
            ...
            e);
        throw new IllegalStateException(
            "Schema migration V" + version + " (" + migration.description() + ") failed", e);
      }
  ...
  private void validateVersionNumbers() {
    for (SchemaMigration migration : migrations) {
      if (migration.version() <= 0) {
        throw new IllegalStateException(
            "Schema migration version must be positive; got "
                + migration.version()
                + " for "
                + migration.description());
      }
