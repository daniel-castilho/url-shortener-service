 ...
 *
 * <p>A forwarded header (default {@code X-Forwarded-For}, left-most entry) is trusted ONLY when the
 * direct peer matches one of the configured trusted-proxy CIDRs — an untrusted client cannot spoof
 * identities by sending the header itself (best practice adopted after reviewing flowtxt and
 * spotpobre; blind header trust is a bypass vector).
 */
@Component
