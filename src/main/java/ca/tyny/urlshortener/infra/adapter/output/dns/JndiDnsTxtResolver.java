...
/**
 * Default {@link DnsTxtResolver} backed by the JDK JNDI DNS provider ({@code jdk.naming.dns}) — no
 * external dependency.
 *
 * <p>Quotes around TXT values are stripped (JNDI returns them quoted). Any lookup failure (name
 * missing, timeout, NXDOMAIN) is treated as "no record", which the verification service reads as a
 * failed check.
 */
@Component
