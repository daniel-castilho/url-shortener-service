 ...
 *
 * <p>Re-verifies every claimed domain (both PENDING/FAILED — promotion, and ACTIVE —
 * revocation detection when the TXT record disappears). Runs via the Spring scheduler and
 * is a no-op when {@code app.domain.dns-verify-enabled} is false (integration tests).
 * Failures for a single domain never abort the run, and the registry is refreshed once
 * the pass completes.
 */
@Component
public class DomainVerificationJob {
    ...
    public DomainVerificationJob(CustomDomainRepositoryPort repository,
