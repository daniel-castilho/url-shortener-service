 ...
 *
 * <p>Authenticated and owner-scoped: a caller only ever sees/manages their own claims. The
 * DNS verification token is returned on creation so the client can publish the TXT record.
 */
@RestController
@RequestMapping("/api/v1/domains")
@Tag(name = "Custom Domains", description = "Claim and manage branded domains (authenticated, owner-scoped)")
public class DomainController {
    ...
    public DomainController(CustomDomainUseCase customDomainUseCase,
    ...
    @PostMapping
    @Operation(summary = "Claim a custom domain", description = "Claims a host for the caller (PENDING) and returns the DNS verification token to publish as a TXT record.")
    @ApiResponses(value = {
    ...
    @GetMapping
    @Operation(summary = "List claimed domains", description = "Returns the caller's custom domains with their verification status.")
    @ApiResponses(value = {
    ...
    public ResponseEntity<DomainListResponse> list() {
        String userId = currentUser.resolveUserId();
        List<DomainResponse> domains = customDomainUseCase.listForOwner(userId).stream()
    ...
    @Operation(summary = "Re-trigger DNS verification", description = "Issues a fresh token, resets the domain to PENDING and runs an immediate DNS check against it.")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Verification re-triggered, result returned"),
            @ApiResponse(responseCode = "400", description = "Invalid host"),
            @ApiResponse(responseCode = "401", description = "Unauthenticated"),
    ...
    public ResponseEntity<DomainResponse> verify(
            @Parameter(description = "Claimed host", example = "links.example.com") @PathVariable String host) {
        String userId = currentUser.resolveUserId();
    ...
    @DeleteMapping("/{host}")
    @Operation(summary = "Remove a custom domain", description = "Drops the caller's claim; links configured with the host fall back to the default-host branch.")
    @ApiResponses(value = {
