...
/**
 * Redis token-bucket rate limiter (Rule 5) — best practices merged from the flowtxt and spotpobre
 * limiters:
 *
 * <ul>
 *   <li><b>Token bucket</b> per (scope, IP): capacity = limit, continuous refill at {@code
 *       limit/window} tokens per second — allows short bursts, smooth sustained rates.
 *   <li><b>Redis TIME</b> drives refill inside one atomic Lua script, so all app instances share a
 *       single clock and N racing instances can never over-admit. No application clock is read.
 *   <li>State is a small hash ({@code tokens, ts}); TTL is {@code max(60s, 2 × full-refill period)}
 *       so idle buckets self-expire — no unbounded key growth under IP rotation.
 *   <li><b>Fail-open</b>: on any Redis failure or unexpected reply the request is allowed and
 *       logged. Throttling must never take down the endpoint it protects; when {@code
 *       rate-limiter.enabled=false} nothing touches Redis.
 * </ul>
 *
 * Raw IPs are stored in bucket keys (deliberate deviation from spotpobre's HMAC-encoded subjects:
 * on-prem single-tenant Redis, keys are already TTL'd).
 */
@Component
public class RedisRateLimiterAdapter implements RateLimiterPort {
  ...
  /**
   * KEYS[1] = bucket key; ARGV: capacity, refillPerSecond. Returns {allowed(0|1),
   * remainingWholeTokens, resetSeconds}.
   */
  ...
  public RedisRateLimiterAdapter(
      StringRedisTemplate redisTemplate, RateLimiterProperties properties) {
  ...
  @io.github.resilience4j.circuitbreaker.annotation.CircuitBreaker(
      name = "rateLimiterCb",
      fallbackMethod = "allowRequestOnFailure")
