 ...
 *
 * <p>Domain claims are unique by host, owner-guarded, and start in PENDING until the DNS health
 * check (infra-side) promotes them to ACTIVE. Pure domain logic — no framework imports.
 */
  ...
  public CustomDomain claim(String userId, String host) {
    ...
    if (normalized.equalsIgnoreCase(defaultHost)) {
      throw new InvalidDomainException(
          "The default host cannot be claimed as a custom domain: " + normalized);
    }
