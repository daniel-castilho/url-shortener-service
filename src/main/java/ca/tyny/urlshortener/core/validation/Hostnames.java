 ...
 *
 * <p>Hosts are stored/compared lowercase, without a trailing dot or port. A valid claimed host is
 * an RFC-1123 hostname containing at least one label separator (a real domain, not a bare
 * single-label host or an IP address).
 */
public final class Hostnames {
  ...
  public static void validate(String normalizedHost) {
    ...
    if (!HOSTNAME.matcher(normalizedHost).matches()
  ...
  /**
   * Normalizes the value of an HTTP {@code Host} header: lower-cases, trims, strips a trailing dot
   * and any {@code :port} suffix. Returns {@code null} for null/blank input.
   */
