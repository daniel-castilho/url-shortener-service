...
public interface ShortenUrlUseCase {
  ...
  /**
   * Shortens a URL with TTL resolution in the application layer. The raw {@code ttlSeconds} is
   * resolved to an expiry instant using the server-side cap.
   */
  ShortUrl shorten(
      String originalUrl, String customAlias, String userId, Long ttlSeconds, String domain);

  // Convenience overloads using expiresAt
  default ShortUrl shorten(
      String originalUrl, String customAlias, String userId, Instant expiresAt) {
