 ...
 *
 * <p>Implemented by the Mongo analytics adapter. Implementations must stay
 * cheap: the daily series and the breakdown read the {@code click_daily}
 * rollup (never the raw event log); only the explicitly requested hourly
 * series touches raw {@code click_events}, and callers bound its range.
 */
public interface ClickAnalyticsPort {
    ...
    /**
     * Ordered hourly buckets (ascending) for the given instant range
     * {@code [fromInclusive, toExclusive)}.
     */
    List<ClicksSeries.Bucket> hourly(String shortCode, Instant fromInclusive, Instant toExclusive);

    /**
     * Per-dimension value-to-count breakdown for {@code [from, to]} inclusive,
