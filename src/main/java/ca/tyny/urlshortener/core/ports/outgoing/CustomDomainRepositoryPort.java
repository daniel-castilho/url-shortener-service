 ...
 *
 * <p>Owned by {@code core/}; implemented by the Mongo adapter. Used by domain management
 * (claim/list/delete/verify), by the shorten/PATCH domain validation, and by the redirect
 * host-aware resolution (via the active-host registry cache).
 */
public interface CustomDomainRepositoryPort {
   ...
   * @throws DomainAlreadyExistsException when the host is already claimed (concurrent claim
   *     duplicate)
   */
