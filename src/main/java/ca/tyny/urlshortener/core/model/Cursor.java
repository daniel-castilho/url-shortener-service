 ...
 *
 * <p>Encodes {@code <epochMillis>:<id>} as Base64url. The client treats it as opaque; the server
 * decodes and validates. Ordering is {@code createdAt} DESC with {@code _id} as tiebreaker (also
 * DESC) to avoid skipping/duplicating when multiple links share the same {@code createdAt}.
 */
