 ...
 *
 * <p>Ownership is proven via a DNS verification record: a {@link #verificationToken()}
 * is returned on claim and the domain enters {@link DomainStatus#ACTIVE} once the record
 * is observed by the scheduled DNS health check. Only ACTIVE domains are usable for
 * shortening and host-aware redirect resolution (option B — a link stores the host it
 * was created for in {@code short_url.domain}).
 */
