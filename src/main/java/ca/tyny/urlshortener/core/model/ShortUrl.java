 ...
 * A shortened link.
 *
 * @param domain the custom host this link is bound to ({@code null} = default host only); a bound
 *     link only redirects when requested on its own host (strict mirror)
 */
public record ShortUrl(
  ...
  /**
   * Copy of this short URL with an updated click count. The count itself is maintained atomically
   * by the persistence adapter ($inc); this accessor exists so reads can expose the current value.
   */
  ...
  /**
   * A short URL is expired when it has an expiry and {@code now} has reached or passed it. The
   * expiry predicate is the application-logic source of truth; the MongoDB TTL index only purges
   * the document later (roughly 60 s cadence).
   *
